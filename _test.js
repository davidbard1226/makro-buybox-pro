...
function loadProducts() {
  ...
  // ── DEDUPLICATE by FSN: keep the richest entry (most fields filled) ──────
  state.products = deduplicateProducts(state.products);

  // ── STRIP bad itm... values incorrectly stored as SKU from old scraper ───
  var skuFixed = false;
...
// Merge duplicate FSN entries — keep the one with the most useful data,
// then copy any non-null fields from the other into it.
function deduplicateProducts(products) {
  var seen = {}; // fsn → index in result
  var result = [];
  products.forEach(function(p) {
    var key = (p.fsn || '').trim().toUpperCase();
    if (!key) {
      // No FSN — keep as-is, can't dedupe without a key
      result.push(p);
      return;
    }
    if (key in seen) {
...
function reEvaluateStatuses() {
  ...
  state.products.forEach(function(p) {
    ...
    // ── MIGRATE FSN from URL if missing ─────────────────────────────────────
...
window.addEventListener('message', function(ev) {
  ...
  if (d.type === 'PRODUCTS_UPDATED') {
    // ── Preserve manually-set fields before reloading from localStorage ──
    // Bridge writes scraped data which won't have SKU/cost/min/max the user set manually.
    var memMap = {};
    state.products.forEach(function(p) {
      var key = (p.fsn||'').toUpperCase() || p.url;
      if (key) memMap[key] = p;
    });
    loadProducts();
    // Merge manual fields back in — never let a bridge write wipe them
    state.products.forEach(function(p) {
      var key = (p.fsn||'').toUpperCase() || p.url;
      var mem = memMap[key];
      if (!mem) return;
      if (mem.sku && !/^itm/i.test(mem.sku) && (!p.sku || /^itm/i.test(p.sku))) p.sku = mem.sku;
      if (mem.cost_price > 0 && !(p.cost_price > 0)) { p.cost_price = mem.cost_price; p.cost_supplier = mem.cost_supplier; }
  ...
  if (d.type === 'SCRAPE_RESULT') {
    // legacy — no longer used in human-mode but kept for compat
    return;
  }
  ...
  if (d.type === 'QUEUE_ABORTED') {
    log('info', '⚠ Queue stopped. ' + (d.done||0) + '/' + (d.total||0) + ' done.');
    finishScraping();
    return;
  }
...
function setExtStatus(online) {
  ...
  document.getElementById('scraperExtLabel').textContent = online ? '✓ Connected — ready to scrape' : 'Waiting for extension...';
...
// ============================================================
// SCRAPER — Human-mode queue (bot-free)
...
function generateUrlsFromListings() {
  ...
  if (!listings.length) {
    statusEl.textContent = '⚠️ No listings imported — go to Products tab and import your Makro Offers XLS first.';
    statusEl.style.color = '#ff9500';
    return;
  }

  // Build FSN → real URL map from already-scraped products
  var fsnToUrl = {};
  state.products.forEach(function(p) {
  ...
  listings.forEach(function(l) {
    ...
    if (fsnToUrl[l.fsn]) {
      // Use the real URL we scraped before — guaranteed to work
  ...
  if (!unique.length) {
    statusEl.textContent = '⚠️ No FSN codes found in listings.';
    statusEl.style.color = '#ff9500';
    return;
  }
  document.getElementById('urlInput').value = unique.join('\n');
  document.getElementById('urlCount').textContent = unique.length + ' URLs loaded';
  statusEl.textContent = '✅ ' + unique.length + ' URLs generated (' + fromHistory + ' direct, ' + fromSearch + ' via search) — click Start Scraping.';
...
function startScraping() {
  ...
  clearLog();
  log('info', '🚀 Human-mode queue started: ' + urls.length + ' URLs');
  log('info', 'Each product page will open in a foreground tab — fully bot-free.');
  log('info', 'Seller: ' + getMySellerName());

  updateProgress(0, urls.length);

  // Send queue to background service worker
  const concurrency = parseInt(document.getElementById('scrapeParallel').value) || 5;
  localStorage.setItem('scrape_parallel', concurrency);
...
function handleQueueProgress(d) {
  ...
  if (d && d.data) {
    ...
    // ── AUTO REPRICE: beat competitor by R1, never below min_price ──────────
    ...
    // ── WHATSAPP ALERT on BuyBox loss ────────────────────────────────────
    if (status === 'lose') sendTelegramAlert(prod, price, seller);

    log(isWin ? 'ok' : 'err',
      (isWin ? '✓ WIN ' : '✗ LOSE') + ' | R' + price.toFixed(2) + ' | ' + seller.slice(0,30) +
      ' | ' + (p.title||'').slice(0,35));
  }
...
function handleQueueFinished() {
  finishScraping();
  log('ok', '✅ Queue complete — all products scraped.');
}
...
// Min price to achieve target profit margin % of selling price after Makro fees
// Makro takes: 10% commission + delivery fee (R50/<1500, R120/1500-3000, R200/>3000)
// Solve: price × (1 - 0.10 - profitPct) = cost + deliveryFee
// => price = (cost + deliveryFee) / (1 - 0.10 - profitPct)
// e.g. 7% margin: price = (cost + 50) / 0.83
function calcMinViablePrice(costPrice) {
  if (!costPrice || costPrice <= 0) return null;
  var pct = getMinProfitPct(); // e.g. 0.07
  var divisor = 1 - 0.10 - pct; // e.g. 0.83 for 7%
  if (divisor <= 0) return null; // sanity check
  var p50  = (costPrice + 50)  / divisor;
  var p120 = (costPrice + 120) / divisor;
  var p200 = (costPrice + 200) / divisor;
  var price;
...
function autoReprice(prod) {
  ...
  if (changed) {
    ...
    log('info',
      '💰 Auto-repriced: R' + roundPrice(buybox) + ' → R' + targetPrice +
      (atFloor ? ' ⚠ AT FLOOR (min R' + minPrice + ')' : ' (beat by R1)')
    );
...
function renderDashboard() {
  ...
  document.getElementById('s-loss-pct').textContent = total > 0 ? Math.round((loses/total)*100) + '% loss rate' : '—% loss rate';

  // Profit summary — products with cost set
  var margins = [];
  ...
  document.getElementById('s-avgmargin').textContent = avgMargin !== null ? avgMargin.toFixed(1) + '%' : '—%';
  ...
  // Low stock warning — from listings
  var listings = getListings();
  ...
  // ── EASY WINS: losses where gap to buybox ≤ R50 ─────────────────────────
  var ewListings = getListings();
  var listMap  = {};
  ewListings.forEach(function(l) { if (l.fsn) listMap[l.fsn] = l; });

  var easyWins = [];
  prods.forEach(function(p) {
    if (p.status !== 'lose' || !p.buybox_price) return;
    var l = listMap[p.fsn];
    var myPrice = l ? l.myPrice : null;
    if (!myPrice) return;
    var gap = myPrice - p.buybox_price;
    if (gap > 0 && gap <= 50) easyWins.push({ p: p, myPrice: myPrice, gap: gap });
  });
  easyWins.sort(function(a, b) { return a.gap - b.gap; });

  var ewEl = document.getElementById('easyWinList');
  var ewCount = document.getElementById('easyWinCount');
  if (ewCount) ewCount.textContent = easyWins.length ? easyWins.length + ' opportunities' : '';
  if (ewEl) {
    if (!easyWins.length) {
      ewEl.innerHTML = '<div style="color:var(--muted);font-size:13px;padding:12px 0">No close losses — either you\'re winning or gap is >R50.</div>';
    } else {
      ewEl.innerHTML = easyWins.slice(0, 6).map(function(e) {
        var beatPrice = e.p.buybox_price - 1;
        var minP = e.p.min_price || 0;
        var canBeat = beatPrice >= minP;
        return '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">' +
          '<div style="flex:1;min-width:0">' +
            '<div style="font-size:12px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="' + escHtml(e.p.title) + '">' + escHtml((e.p.title||'').slice(0,38)) + '</div>' +
            '<div style="font-size:11px;color:var(--muted);margin-top:2px">' +
              'My: <span style="color:#ff9500">R' + e.myPrice.toFixed(0) + '</span> · ' +
              'BuyBox: <span style="color:var(--accent)">R' + e.p.buybox_price.toFixed(0) + '</span> · ' +
              'Gap: <span style="color:#ffd60a">R' + e.gap.toFixed(0) + '</span>' +
            '</div>' +
          '</div>' +
          '<div style="text-align:right;flex-shrink:0">' +
            (canBeat
              ? '<span style="font-family:monospace;font-size:12px;color:var(--accent);font-weight:700">→ R' + beatPrice + '</span>'
              : '<span style="font-size:10px;color:var(--muted)">below min<br>R' + minP.toFixed(0) + '</span>') +
          '</div>' +
        '</div>';
      }).join('');
    }
  ...
  var todayWins = 0, todayLoses = 0, todayChecked = 0;
  var competitorToday = {};
  prods.forEach(function(p) {
    (p.history || []).forEach(function(h) {
      if (!h.ts || h.ts.slice(0, 10) !== todayStr) return;
      todayChecked++;
      if (h.status === 'win') todayWins++;
      else if (h.status === 'lose') {
        todayLoses++;
        if (h.seller && h.seller !== 'Unknown Seller') {
          competitorToday[h.seller] = (competitorToday[h.seller] || 0) + 1;
        }
      }
    });
  });

  var topComp = Object.entries(competitorToday).sort(function(a, b) { return b[1] - a[1]; }).slice(0, 3);
  var todayRate = todayChecked ? Math.round(todayWins / todayChecked * 100) : null;

  var tpEl = document.getElementById('todayPerf');
  if (tpEl) {
    if (!todayChecked) {
      tpEl.innerHTML = '<div style="color:var(--muted);font-size:13px;padding:12px 0">No scrapes yet today. Run the scraper to see today\'s results.</div>';
    } else {
      var rateCol = todayRate >= 50 ? 'var(--accent)' : todayRate >= 25 ? '#ffd60a' : '#ff4d6d';
      tpEl.innerHTML =
        '<div style="display:flex;gap:20px;margin-bottom:14px">' +
          '<div style="text-align:center">' +
            '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:36px;color:var(--accent);line-height:1">' + todayWins + '</div>' +
            '<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Wins</div>' +
          '</div>' +
          '<div style="text-align:center">' +
            '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:36px;color:#ff4d6d;line-height:1">' + todayLoses + '</div>' +
            '<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Losses</div>' +
          '</div>' +
          '<div style="text-align:center">' +
            '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:36px;color:' + rateCol + ';line-height:1">' + (todayRate !== null ? todayRate + '%' : '—') + '</div>' +
            '<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Win Rate</div>' +
          '</div>' +
          '<div style="text-align:center">' +
            '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:36px;color:var(--muted);line-height:1">' + todayChecked + '</div>' +
            '<div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1px">Checked</div>' +
          '</div>' +
        '</div>' +
        (topComp.length ? '<div style="font-size:11px;color:var(--muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:1px">Top competitors stealing today</div>' +
          topComp.map(function(c) {
            return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px">' +
              '<div style="flex:1;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--accent2)">' + escHtml(c[0]) + '</div>' +
              '<div style="font-family:monospace;font-size:12px;color:var(--muted)">' + c[1] + ' losses</div>' +
            '</div>';
          }).join('') : '<div style="font-size:12px;color:var(--accent)">✓ No losses to competitors today!</div>');
    }
  }

  // Warning cards
  var noSku     = prods.filter(function(p){ return !p.sku; }).length;
  var noCost    = prods.filter(function(p){ return !(p.cost_price > 0); }).length;
  var pending   = prods.filter(function(p){ return p.pending_price != null; }).length;
  var wNoSku    = document.getElementById('w-nosku');
  var wNoCost   = document.getElementById('w-nocost');
  var wPending  = document.getElementById('w-pending');
  var wLosing   = document.getElementById('w-losing');
  if (wNoSku)   { wNoSku.textContent   = noSku;  wNoSku.closest('.stat-card').style.display   = noSku   > 0 ? '' : 'none'; }
...
function renderProducts() {
  ...
  let prods = state.products.filter(p => {
    ...
    if (filter === 'no-cost' && p.cost_price > 0) return false;
    if (filter === 'pending' && p.pending_price == null) return false;
  ...
  // Update filtered count in toolbar
  var countEl = document.getElementById('filteredCount');
  if (countEl) {
    var filterVal = document.getElementById('prodFilter').value;
    var searchVal = (document.getElementById('prodSearch').value||'').trim();
    if (filterVal !== 'all' || searchVal) {
      countEl.textContent = totalProds + ' of ' + state.products.length + ' products';
    } else {
      countEl.textContent = '';
    }
  }
  ...
  // Build a stable index map: filtered+paged product → index in state.products
  ...
  tbody.innerHTML = pageProds.map((p, i) => {
    ...
    const statusLabel = p.status === 'win' ? '✓ WIN' : p.status === 'lose' ? '✗ LOSE' : '? UNKNOWN';
    const t = p.lastChecked ? new Date(p.lastChecked).toLocaleString() : '—';
    const realIdx = findRealIdx(p);
    const listing = listingsMap[p.fsn] || null;
    const myPrice = listing ? `<span style="color:var(--accent3);font-family:'Space Mono',monospace;font-size:12px">R ${listing.myPrice.toFixed(2)}</span>` : '<span style="color:var(--muted);font-size:11px">—</span>';
    const myStock = listing ? `<span style="color:${listing.myStock > 0 ? 'var(--accent)' : '#ff9500'};font-size:12px">${listing.myStock}</span>` : '<span style="color:var(--muted);font-size:11px">—</span>';
    let priceIndicator = '';
    if (listing && p.buybox_price > 0) {
      if (listing.myPrice > p.buybox_price) priceIndicator = ' <span style="color:#ff9500;font-size:10px">▲above</span>';
      else if (listing.myPrice < p.buybox_price) priceIndicator = ' <span style="color:var(--accent);font-size:10px">▼below</span>';
    ...
    return `<tr>
      ...
      <td>
        <input type="text"
          ...
          title="Makro FSN / product ID — click to edit"
      ...
      <td>
        ${p.sku
          ? `<span style="font-family:'Space Mono',monospace;font-size:12px;color:#818cf8;font-weight:600">${escHtml(p.sku)}</span>
             <button class="action-btn" style="font-size:9px;padding:1px 5px;margin-left:3px" onclick="openEditModal(${realIdx})" title="Edit SKU/Cost/Min">✎</button>`
      ...
      <td>
        ${p.cost_price > 0
          ? `<div style="display:flex;align-items:center;gap:4px">
               ...
               <button class="action-btn" style="font-size:9px;padding:1px 5px" onclick="openEditModal(${realIdx})" title="Edit cost">✎</button>
             </div>`
      ...
      <td>
        ${(() => {
          ...
          if (!cost || cost <= 0) return '<span style="color:var(--muted);font-size:10px">—</span>';
          ...
          return lines.length ? lines.join('') : '<span style="color:var(--muted);font-size:10px">—</span>';
        })()}
      </td>
      <td>
        ${(() => {
          // New Price column — shows pending reprice or calculated suggested price
          var bb = p.buybox_price;
          var minP = p.min_price || 0;
          var maxP = p.max_price || null;
          var myP = (listingsMap[p.fsn] || {}).myPrice || null;

          // If a reprice is already queued (auto-reprice ran after last scrape)
          if (p.pending_price != null) {
            var diff = myP ? (p.pending_price - myP) : null;
            var diffStr = diff !== null ? (diff < 0 ? ' <span style="color:var(--accent);font-size:10px">▼R'+Math.abs(diff)+'</span>' : ' <span style="color:#ff9500;font-size:10px">▲R'+diff+'</span>') : '';
            return '<div style="display:flex;flex-direction:column;gap:2px">' +
              '<span style="font-family:monospace;font-size:13px;font-weight:700;color:var(--accent3)">R'+p.pending_price+'</span>' +
              '<span style="font-size:9px;color:var(--accent3);opacity:.8">⏳ pending'+diffStr+'</span>' +
              '</div>';
          }
          // No pending — calculate what it would be if repriced now
          if (!bb || bb <= 0 || p.status === 'win') return '<span style="color:var(--muted);font-size:10px">—</span>';
          var suggested = bb - 1;
          var clamped = false;
          var clampNote = '';
          if (minP > 0 && suggested < minP) { suggested = minP; clamped = true; clampNote = 'at floor'; }
          if (maxP && suggested > maxP)     { suggested = maxP; clamped = true; clampNote = 'at ceiling'; }
          var sameAsMyPrice = myP && suggested === Math.round(myP);
          if (sameAsMyPrice) return '<span style="color:var(--muted);font-size:10px">no change</span>';
          var col = clamped ? '#ffd60a' : 'var(--accent)';
          var diff2 = myP ? (suggested - myP) : null;
          var diffStr2 = diff2 !== null ? (diff2 < 0 ? ' <span style="color:var(--accent);font-size:10px">▼R'+Math.abs(diff2)+'</span>' : ' <span style="color:#ff9500;font-size:10px">▲R'+diff2+'</span>') : '';
          return '<div style="display:flex;flex-direction:column;gap:2px">' +
            '<span style="font-family:monospace;font-size:13px;color:'+col+'">R'+suggested+'</span>' +
            '<span style="font-size:9px;color:var(--muted)">'+(clamped ? '⚠ '+clampNote : '💡 suggested')+diffStr2+'</span>' +
            '</div>';
        })()}
      </td>
      ...
      <td style="display:flex;gap:4px">
        <button class="action-btn" onclick="showHistory(${realIdx})" title="Price history">📈</button>
        <button class="action-btn" onclick="rescrapeOne(${realIdx})" title="Re-scrape">↻</button>
        <button class="action-btn danger" onclick="deleteProduct(${realIdx})" title="Delete">✕</button>
      </td>
    </tr>`;
  }).join('');
  ...
  if (pagEl) {
    if (totalPages <= 1) { pagEl.innerHTML = ''; }
      ...
      let html = `<span style="font-size:12px;color:var(--muted)">${pageStart+1}–${Math.min(pageStart+pageSize,totalProds)} of ${totalProds}</span> `;
      html += `<button ${btnStyle(false)} onclick="prodPage=Math.max(1,prodPage-1);renderProducts()" ${prodPage===1?'disabled':''}>‹ Prev</button>`;
      ...
      html += `<button ${btnStyle(false)} onclick="prodPage=Math.min(totalPages,prodPage+1);renderProducts()" ${prodPage===totalPages?'disabled':''}>Next ›</button>`;
...
function importListings(input) {
  ...
  // Step 1: read as Data URL (no stack overflow risk — browser handles it natively)
  const urlReader = new FileReader();
  urlReader.onload = function(urlEvt) {
    // urlEvt.target.result is "data:...;base64,XXXX" — extract the base64 part
  ...
  reader.onload = function(e) {
    try {
      ...
      // dense:true uses array-of-arrays internally — avoids deep recursion on large XLS files
      ...
      // ── Stamp real SKU onto matched products via FSN ──────────────────────
      const fsnToSku = {};
      ...
      alert('✅ Imported ' + listings.length + ' listings!' + (skuCount > 0 ? ' Matched ' + skuCount + ' SKU(s) to products.' : ''));
      renderProducts();
    } catch(err) {
  ...
  try {
    ...
    state.products.forEach(function(p) {
      ...
      if (match) {
        ...
        // Always recalculate min price from current profit % setting + cost
        p.min_price = calcMinViablePrice(match.cost);
        // Only use supplier max if they provide one, otherwise auto-calc 20% above min
        if (match.maxPrice != null) {
          p.max_price = match.maxPrice;
        } else {
          // Always refresh max when re-syncing costs
    ...
    var msg = '✅ ' + updated + ' product(s) updated with cost data.';
    if (noSku > 0)   msg += '\n⚠ ' + noSku + ' product(s) have no SKU — use 🔗 Bulk SKU Import or ⬇ Export Unmatched to fix.';
    if (noMatch.length > 0) msg += '\n📋 ' + noMatch.length + ' SKU(s) not found in any supplier sheet:\n' + noMatch.slice(0,10).join(', ') + (noMatch.length > 10 ? '...' : '');
    msg += '\n\n💡 Total supplier SKUs loaded: ' + Object.keys(lookup).length;
    alert(msg);
...
// Recalculate min (and max) prices for all products that have a cost price,
// using the current min_profit_pct setting. Call after changing the profit % slider.
function recalcAllMinPrices() {
  var pct = getMinProfitPct() * 100;
  var updated = 0;
  state.products.forEach(function(p) {
    if (!p.cost_price || p.cost_price <= 0) return;
    p.min_price = calcMinViablePrice(p.cost_price);
    p.max_price = calcAutoMaxPrice(p.min_price);
    updated++;
  });
  if (updated > 0) {
    saveProducts();
    renderProducts();
    alert('✅ Recalculated min/max prices for ' + updated + ' products at ' + pct.toFixed(0) + '% min profit.');
  } else {
    alert('⚠ No products with cost prices found — sync costs first.');
  }
}

function deleteProduct(idx) {
  if (idx < 0 || idx >= state.products.length) {
    alert('Could not find that product — try refreshing the page.');
    return;
  }
...
function updateEditCalcPreview() {
  ...
  preview.innerHTML =
    ...
    '→ <strong style="color:var(--accent3)">Min = R' + autoMin + '</strong> &nbsp;&nbsp; ' +
    '<strong style="color:#a78bfa">Max = R' + effectiveMax + '</strong> <span style="color:var(--muted)">(20% above min)</span>';

  if (minHint) minHint.textContent = 'Auto: R' + autoMin + ' — override by typing above';
  if (maxHint) maxHint.textContent = 'Auto: R' + effectiveMax + ' — override by typing above';
}
...
function saveEditModal() {
  ...
  // If product has a cost but min_price is missing or zero, always auto-calc it
  if (p.cost_price > 0 && (!p.min_price || p.min_price <= 0)) {
    p.min_price = calcMinViablePrice(p.cost_price);
  }
...
// Legacy — no longer used (modal replaces it) but kept to avoid JS errors
...
function handleSearchInput(val) {
  ...
  var isUrl = val.trim().startsWith('http') && val.trim().includes('makro.co.za');
  if (btn) btn.style.display = isUrl ? 'flex' : 'none';
}

function addProductFromSearch() {
  var url = (document.getElementById('prodSearch').value || '').trim();
  if (!url.startsWith('http') || !url.includes('makro.co.za')) {
    alert('Please paste a valid makro.co.za product URL.');
    return;
  }
  // Check not already tracked
  var exists = state.products.some(function(p) { return p.url === url; });
  if (exists) {
    alert('⚠ This product is already in your list.');
    return;
  }
  // Add as a stub — scraper will fill in the rest
  var stub = {
    url: url,
    fsn: '', sku: '', title: url,
    buybox_price: 0, seller: '', status: 'unknown',
    lastChecked: null, history: []
  };
  state.products.push(stub);
  saveProducts();
  renderProducts();
  updateProductCount();
  ...
  // Ask if they want to scrape it now
  if (state.extReady && confirm('✅ Product added! Scrape it now to get BuyBox data?')) {
    switchTab('scraper');
...
// Export products with no SKU match so the user can fix & re-upload
function exportUnmatchedCSV() {
  var unmatched = state.products.filter(function(p) { return !p.sku; });
  if (unmatched.length === 0) { alert('✅ All products have a matched SKU — nothing to export!'); return; }
  // Headers: FSN (to find the product), Title (for reference), blank SKU column for user to fill
  var headers = ['FSN', 'Title', 'SKU (fill this in)', 'BuyBox Price', 'Status', 'Last Checked'];
  var rows = unmatched.map(function(p) {
    return [p.fsn||'', p.title||'', '', p.buybox_price||'', p.status||'', p.lastChecked||'']
      .map(function(v){ return '"' + String(v).replace(/"/g,'""') + '"'; });
  });
  var csv = [headers.map(function(h){ return '"'+h+'"'; }), ...rows].map(function(r){ return r.join(','); }).join('\n');
  var blob = new Blob([csv], { type: 'text/csv' });
  var a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'makro-unmatched-skus-' + new Date().toISOString().slice(0,10) + '.csv';
  a.click();
  alert('📥 Exported ' + unmatched.length + ' unmatched products.\n\nFill in the "SKU (fill this in)" column, then use 🔗 Bulk SKU Import to re-upload.');
}
...
function renderPriceComp(prods) {
  ...
  if (!el) return;  // element not present — skip silently
...
// ── Status bar chart (now in Analytics tab) ──────────────────────────────
...
function renderCompetitors(prods) {
  ...
  el.innerHTML = sorted.map(function([name, d]) {
    ...
    const avg   = d.prices.length ? 'R' + (d.prices.reduce((a,b)=>a+b,0)/d.prices.length).toFixed(0) : '—';
...
// ── Price Gap Analysis — how far are you from winning ────────────────────
function renderPriceGap(prods) {
  ...
  if (!gaps.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:12px;line-height:1.6">No price gap data yet — make sure your S_listing FSN codes match the scraped products.</div>';
    return;
  }

  gaps.sort((a, b) => Math.abs(a.gap) - Math.abs(b.gap));

  const buckets = { '<R10':0, 'R10–50':0, 'R50–200':0, 'R200–500':0, '>R500':0 };
  gaps.forEach(function(g) {
    const a = Math.abs(g.gap);
    if (a < 10)       buckets['<R10']++;
    else if (a < 50)  buckets['R10–50']++;
    else if (a < 200) buckets['R50–200']++;
    else if (a < 500) buckets['R200–500']++;
    else              buckets['>R500']++;
  });
...
// ── Top Opportunities — products closest to winning buybox ────────────────
function renderOpportunities(prods) {
  ...
  el.innerHTML = `<table style="width:100%;border-collapse:collapse;font-size:12px">
    ...
    top.map(function({ p, myPrice, gap }) {
      const gapStr  = gap !== null ? (gap > 0 ? '<span style="color:#ff9500">▲R'+gap.toFixed(2)+'</span>' : '<span style="color:#00e5a0">▼R'+Math.abs(gap).toFixed(2)+'</span>') : '<span style="color:var(--muted)">—</span>';
      const myPriceStr = myPrice ? 'R'+myPrice.toFixed(2) : '<span style="color:var(--muted)">—</span>';
      ...
      return `<tr style="border-bottom:1px solid var(--border)${closeEnough ? ';background:rgba(0,229,160,0.04)' : ''}">
        ...
        <td style="text-align:center;padding:8px 6px"><a href="${escHtml(p.url)}" target="_blank" style="color:var(--accent);font-size:11px;text-decoration:none">Open ↗</a></td>
      </tr>`;
    }).join('') +
    '</tbody></table>';
}

// ── Summary Stats — enriched ──────────────────────────────────────────────
function renderSummary(prods) {
  ...
  try {
    ...
    const rows = [
      ...
      ['Lowest BuyBox Price',  min > 0 ? 'R ' + min.toFixed(2) : '—', ''],
      ['Highest BuyBox Price', max > 0 ? 'R ' + max.toFixed(2) : '—', ''],
      ['Unique Competitors',   Object.keys(sellers).length,       ''],
      ['Top Competitor',       topSeller ? topSeller[0] : '—',    'color:var(--accent2);font-size:11px;text-align:right;max-width:160px'],
...
(function init() {
  ...
  var savedParallel = localStorage.getItem('scrape_parallel') || '5';
  var pEl = document.getElementById('scrapeParallel');
  if (pEl) pEl.value = savedParallel;
  initAutoScrape();
  ...
  setInterval(function() {
    ...
    try {
      ...
      if (cleaned.length !== lastProductCount || currentScrape !== lastScrapeTime) {
        ...
        // ── PRESERVE manually-set SKU, cost, min/max price from in-memory state ──
        // The bridge may write stale data without these fields — never let a
        // localStorage reload wipe fields the user just typed in.
        var memMap = {};
        state.products.forEach(function(p) {
          var key = (p.fsn||'').toUpperCase() || p.url;
          if (key) memMap[key] = p;
        ...
        cleaned.forEach(function(p) {
          ...
          var mem = memMap[key];
          if (!mem) return;
          // Keep manual SKU if bridge gives nothing or a bad itm... slug
          if (mem.sku && !/^itm/i.test(mem.sku) && (!p.sku || /^itm/i.test(p.sku))) p.sku = mem.sku;
          // Keep cost + pricing if already set in memory
          if (mem.cost_price > 0 && !(p.cost_price > 0)) {
            p.cost_price    = mem.cost_price;
            p.cost_supplier = mem.cost_supplier;
          }
          if (mem.min_price > 0 && !(p.min_price > 0)) p.min_price = mem.min_price;
          if (mem.max_price > 0 && !(p.max_price > 0)) p.max_price = mem.max_price;
        });
...
function initPricer() {
  ...
  if (listings.length === 0) {
    el.innerHTML = '⚠️ <span style="color:#ff9500">No listings imported yet.</span> Go to <b>Products tab → 📂 Import Listings</b> first.';
    document.getElementById('generateBtn').disabled = true;
  } else {
    el.innerHTML = '✅ <span style="color:var(--accent)">' + listings.length + ' listings loaded.</span> Ready to generate.';
...
function calcNewPrice(myPrice, buyboxPrice, strategy, beatBy, floorPct, minPrice, maxPrice) {
  ...
  // Round to whole number — Makro prices should be whole Rands
  newPrice = roundPrice(newPrice);

  // Apply min/max guards — return object so callers can detect clamping
...
function buildPricerPreview() {
  ...
  listings.forEach(function(l) {
    ...
    if (!scraped) return;
    // Safety guard — skip products with no cost or no min price
    if (!(scraped.cost_price > 0) || !(scraped.min_price > 0)) return;
  ...
  displayRows.forEach(function(r) {
    ...
    const clampBadge  = r.clamped ? ' <span style="color:#ff9500;font-size:9px">⚠ ' + r.clampDir.toUpperCase() + '</span>' : '';
...
function generatePriceUpdate() {
  ...
  if (!rawBase64) {
    document.getElementById('pricerResult').innerHTML =
      '❌ <span style="color:#ff4444">Original listing file not found.<br>Please re-import your S_listing XLS file from the Products tab — the raw file is needed as a template.</span>';
    return;
  }

  // Build FSN → new price map
  const scrapedMap = {};
  state.products.forEach(function(p) { if (p.fsn) scrapedMap[p.fsn] = p; });
  ...
  listings.forEach(function(l) {
    ...
    // ── SAFETY GUARD: never reprice without cost + min price set ────────────
    // Without these we can't know if the new price is profitable.
    if (!(scraped.cost_price > 0) || !(scraped.min_price > 0)) {
      skippedNoCost++;
      return;
    }
  ...
  if (updateCount === 0) {
    document.getElementById('pricerResult').innerHTML = '⚠️ No products to update with current settings.';
    return;
  }

  // Load the original workbook as template — keep ALL sheets, ALL columns, ALL rows intact
  ...
  try {
    ...
    document.getElementById('pricerResult').innerHTML =
      '✅ <span style="color:var(--accent)">Downloaded <b>' + filename + '</b> — ' +
      changed + ' prices updated.' +
      (skippedNoCost ? ' <span style="color:#818cf8">⚠ ' + skippedNoCost + ' skipped — no cost/min price set (safety guard).</span>' : '') +
      (skippedClamped ? ' <span style="color:#ff9500">' + skippedClamped + ' skipped (price guard).</span>' : '') +
      '</span><br>' +
      '<span style="color:var(--muted)">Click <b>🚀 Open Seller Portal</b> to auto-upload.</span>' +
      ...
      changeLog.map(function(c) {
        ...
        return '<tr style="border-bottom:1px solid var(--border)">' +
          '<td style="padding:4px;font-family:monospace;color:#818cf8">' + escHtml(c.sku||'—') + '</td>' +
          '<td style="padding:4px;font-family:monospace;font-size:10px;color:var(--muted)">' + escHtml(c.fsn||'—') + '</td>' +
    ...
    document.getElementById('pricerResult').innerHTML =
      '❌ <span style="color:#ff4444">Error building file: ' + err.message + '<br>Please re-import your listing file.</span>';
  }
}
...
function sendFileToPortal() {
  ...
  if (!base64) {
    alert('No price file generated yet.\nGo to Price Updater → Generate & Download first.');
    return;
  }
  // Send to bridge → chrome.storage.local so portal.js can read it
  window.postMessage({ type: 'SAVE_PORTAL_FILE', base64, filename }, '*');
  setTimeout(function() {
    document.getElementById('pricerResult').innerHTML =
      (document.getElementById('pricerResult').innerHTML || '') +
      '<br><span style="color:#ffd60a">✅ File sent to portal extension. Open seller.makro.co.za — click <b style="color:#00e5a0">🚀 Auto Upload Now</b> in the green overlay.</span>';
  }, 300);
}
...
function saveTgSettings() {
  var token  = (document.getElementById('tgBotToken') || {}).value || '';
  var chatId = (document.getElementById('tgChatId')   || {}).value || '';
  var enabled= !!(document.getElementById('tgEnabled') || {}).checked;
  if (token)  localStorage.setItem('tg_token',   token);
  if (chatId) localStorage.setItem('tg_chat_id', chatId);
  localStorage.setItem('tg_enabled', enabled ? '1' : '0');
}

function loadTgSettings() {
  var tEl = document.getElementById('tgBotToken');
  var cEl = document.getElementById('tgChatId');
  var eEl = document.getElementById('tgEnabled');
  if (tEl) tEl.value   = localStorage.getItem('tg_token')   || '';
  if (cEl) cEl.value   = localStorage.getItem('tg_chat_id') || '';
  if (eEl) eEl.checked = localStorage.getItem('tg_enabled') === '1';
}
...
document.addEventListener('change', function(e) {
  if (e.target && ['tgBotToken','tgChatId','tgEnabled'].includes(e.target.id)) saveTgSettings();
});
...
function addTestLogEntry() {
  addPriceLogEntry({
    ts: new Date().toISOString(),
    title: 'Test Product — Storage Check',
    fsn: 'TESTFSN123456',
    sku: 'TEST-SKU-001',
    status: 'lose',
    buybox_price: 999.00,
...
function renderPriceLog() {
  ...
  if (!log.length) {
    el.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><div class="empty-title">No Log Yet</div><div class="empty-text">Price changes appear here after scraping.</div></div>';
    return;
  }
  var rows = log.slice(0, 500).map(function(e) {
    ...
    var si = e.status === 'win' ? '✓' : e.status === 'lose' ? '✗' : '?';
    return '<tr><td style="padding:7px 10px;color:var(--muted);white-space:nowrap;font-size:11px">' + d + '</td>' +
      ...
      '<td style="padding:7px 10px;font-family:monospace;font-size:11px;color:#818cf8">' + escHtml(e.sku||'—') + '</td>' +
      ...
      '<td style="padding:7px 10px;font-family:monospace;color:#60a5fa">' + (e.my_price ? 'R' + Number(e.my_price).toFixed(2) : '—') + '</td>' +
      '<td style="padding:7px 10px;font-family:monospace;color:var(--accent3)">' + (e.pending_price ? 'R' + e.pending_price : '—') + '</td>' +
...
// ============================================================
// BULK SKU IMPORT — paste, CSV file, or Excel file
// ============================================================
...
function processBulkSkuFile(file) {
  ...
  document.getElementById('bulkSkuResult').textContent = '⏳ Reading file...';
  ...
  if (name.endsWith('.csv')) {
    ...
    reader.onload = function(e) {
      try {
        ...
        document.getElementById('bulkSkuResult').textContent = '❌ Could not read Excel file: ' + err.message;
      }
    };
    reader.readAsArrayBuffer(file);
  } else {
    document.getElementById('bulkSkuResult').textContent = '❌ Unsupported file type. Use .csv, .xls or .xlsx.';
  }
}
...
function parseBulkSkuText(text) {
  ...
  lines.forEach(function(line) {
    ...
    if (!a || !b) return;
    // Skip header rows — FSN is uppercase alphanumeric 10-20 chars
...
function showBulkSkuPreview(pairs, filename) {
  ...
  if (!pairs.length) {
    resEl.textContent = '⚠ No valid SKU/FSN pairs found in ' + filename;
    prevEl.textContent = '';
    return;
  }
  ...
  prevEl.textContent = '📄 ' + filename + ' — ' + pairs.length + ' rows parsed, ' + matchable + ' match existing products';
  resEl.textContent = '';
}

function saveBulkSkus() {
  var raw = document.getElementById('bulkSkuInput').value.trim();
  if (!raw) { document.getElementById('bulkSkuResult').textContent = '⚠ Nothing to import.'; return; }

  var pairs = parseBulkSkuText(raw);
  if (!pairs.length) {
    document.getElementById('bulkSkuResult').textContent = '⚠ No valid SKU,FSN rows found. Check format.';
    return;
  }
  ...
  document.getElementById('bulkSkuResult').innerHTML =
    '<span style="color:var(--accent)">✅ Imported ' + pairs.length + ' rows — matched and saved SKUs for ' + matched + ' products.</span>';
  setTimeout(function() {
    document.getElementById('bulkSkuModal').classList.remove('show');
    renderProducts();
  }, 1400);
...
function buildAutoScrapeQueue() {
  ...
  if (mode === 'losses') {
    ...
    if (!urls.length) {
      // Nothing to lose — scrape everything to refresh
      urls = state.products.filter(function(p){ return p.url; }).sort(byAge).map(function(p){ return p.url; });
    }
  } else if (mode === 'all') {
    urls = state.products.filter(function(p){ return p.url; }).sort(byAge).map(function(p){ return p.url; });
  } else {
    // Smart: losses first, then unknown, then wins — all included
...
function applyAutoScrape() {
  ...
  function showStatus(nextMs) {
    ...
    statusEl.innerHTML =
      '⏰ Next run: <strong>' + new Date(nextMs).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) + '</strong>' +
      ' &nbsp;·&nbsp; Mode: ' + modeLabel +
  ...
  _autoScrapeTimer = setInterval(function() {
    // If previous cycle is still running, skip this tick — don't interrupt it
    if (state.scraping) {
      log('info', '⏭ Auto-scrape tick skipped — previous cycle still running.');
    ...
    _autoScrapeRun++;
    if (!state.extReady) {
      log('info', '⏸ Auto-scrape skipped — extension offline.');
      nextRun = Date.now() + mins * 60000;
      showStatus(nextRun);
      return;
    }
    ...
    if (!urls.length) {
      log('info', '⏸ Auto-scrape skipped — no products with URLs.');
      nextRun = Date.now() + mins * 60000;
      showStatus(nextRun);
      return;
    }
    ...
    log('info', '🤖 Auto-scrape #' + _autoScrapeRun + ' started — ' + urls.length + ' products' +
      (mode === 'smart' ? ' (' + losses + ' losses first, then ' + wins + ' wins)' : ' [' + mode + ']'));

    document.getElementById('urlInput').value = urls.join('\n');
    var concurrency = parseInt(localStorage.getItem('scrape_parallel') || (document.getElementById('scrapeParallel')||{}).value || '5');
    window.postMessage({ type: 'START_QUEUE', urls: urls, concurrency: concurrency }, '*');
...
function openPortalUpload() {
  ...
  // Send to bridge → chrome.storage.local
  window.postMessage({ type: 'SAVE_PORTAL_FILE', base64, filename }, '*');
  ...
  document.getElementById('pricerResult').innerHTML =
    (document.getElementById('pricerResult').innerHTML || '') +
    '<br><span style="color:#ffd60a">✅ File sent. On the portal, click <b style="color:#00e5a0">🚀 Auto Upload Now</b> in the green overlay.</span>';
}
